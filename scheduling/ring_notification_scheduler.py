...
class RingNotificationScheduler:
    def __init__(self, user_service: UserService,
        ...
        self.ring_service = ring_service
        self.tg_bot = tg_bot

    def schedule(self):
        now = datetime.now(ZoneInfo("UTC"))
        for user in self.user_service.retrieve_all_users():
    ...
    def __handle_change_and_notify(self, ring_status: RingStatusEnum,
        ...
        self.ring_service.update_ring_date(chat_id=user.user.chat_id,
                                           ring_date=datetime.now().strftime("%Y-%m-%d"))
        self.ring_service.update_ring_status(chat_id=user.user.chat_id,
                                             ring_status=ring_status)
        if RingStatusEnum.INSERTED == ring_status:
            self.tg_bot.send_message(chat_id=user.user.chat_id,
                                     text="Time to insert ring",
                                     parse_mode=constant_bean.parser())
        else:
            self.tg_bot.send_message(chat_id=user.user.chat_id,
                                     text="Time to remove ring",
                                     parse_mode=constant_bean.parser())
